...
The two defences that actually matter are elsewhere: the anti-fabrication clause in `agents/code-review-claude.md` ("Earn the verdict" — every verdict backed by work done, and *"I could not verify"* a first-class `concern` answer), and the harness warning. When the task-completion notification carries `[Self Approval]` or any flag about the subagent's own output, **the file is not evidence** — step 5c must insert `review-loop-failure` and skip ingest. That warning arrives only in the notification text; no code in the loop can see it.
